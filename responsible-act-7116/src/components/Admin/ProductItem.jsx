...
} from "@/redux/allProduct/allProduct.action";
...
const ProductItem = ({
  ...
  const handleDelete = () => {
    dispatch(deleteAllProductData(id, type)).then(() =>
      toast({
...
export function UserProfileEdit({ img, titl, prc, type, id, justUpdate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    dispatch(updateAllProductData(id, data, type))
      .then(() =>
        toast({
          position: "bottom",
          render: () => (
          ...
          <Stack direction={"column"} spacing={6}>
            <Center>
              <Image
                w="100px"
                borderRadius={"10px"}
                src={`${img.furl + img.version.v3 + img.burl}`}
              ></Image>
            </Center>
            <Center w="full">
